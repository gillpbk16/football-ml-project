...
def create_odds_features(home_team: str, away_team: str, 
                            prob_home: float = 0.4,
                            prob_draw: float = 0.3, 
                            prob_away: float = 0.3):
...
def create_teamstats_features(home_goals_for: float = 1.5, home_goals_against: float = 1.5, 
                                away_goals_for: float = 1.5, away_goals_against: float = 1.5):
...
async def predict_match(request: PredictionRequest):
    ...
    try:
        ...
        if abs(total_prob - 1.0) > 0.1:
            raise HTTPException(status_code = 400, detail="Probabilities should sum to approximately 1.")
