...
from src.config import (DATA_DIR, MODELS_DIR, MODEL_VERSION, MODELS, LABEL_MAPPING, TARGET_CLASSES, DATASETS)
...
def prepare_features_labels(train_df, test_df, dataset_type):
    ...
    if train_y.isnull().any() or test_y.isnull().any():
        ...
        raise ValueError(f"Unmapped labels found. Train: {bad_train}  Test: {bad_test}. "
    ...
    if dataset_type == "teamstats":
        ...
        feature_cols = [col for col in feature_cols if pd.api.types.is_numeric_dtype(train_df[col])]
