...
def evaluate_baselines():
    ...
    _, _, train_y_odds, test_y_odds, _ = prepare_features_labels(
        train_df_odds, test_df_odds, "odds_teams"
    )
    ...
    for baseline_name, baseline in baselines.items():
        ...
        assert probs.shape[1] == len(
            TARGET_CLASSES
        ), "baseline probs must be (n, 3) in H,D,A order"

        preds = np.argmax(probs, axis=1)
        metrics = {
            "accuracy": accuracy_score(test_y_odds, preds),
            "log_loss": log_loss(test_y_odds, probs),
            "baseline_improvement": accuracy_score(test_y_odds, preds)
            - RANDOM_BASELINE,
    ...
    for dataset_type in ("teamstats", "odds_teams"):
        ...
        for family in ("logreg", "rf"):
            ...
            if not model_path.exists() or not meta_path.exists():
                print(
                    f"[skip] Missing artifacts for {dataset_type}/{family}/{MODEL_VERSION}"
                )
                continue
